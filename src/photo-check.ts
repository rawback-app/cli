...
export async function runPhotoCheck(
  ...
  const ui = commandOutput(dependencies)
  const activity = ui.startActivity('Scanning local photos…', !options.json)
  let report: PhotoCheckReport
  try {
    const client = dependencies.client ?? (await createCommandClient(dependencies))
    const files = await scanUploadPath(options.path)
    ...
    activity.update('Reading capture metadata…')
    ...
    try {
      ...
      activity.update('Checking Rawback…')
      await checkRemoteIdentities(client, identities, results)
    } catch {
