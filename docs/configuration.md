...
Running `rawback auth` creates `~/.rawback/credentials.json` with an access token
and refresh token. The write is atomic, and the directory and file receive
restrictive permissions on Unix. Do not edit this file by hand.
